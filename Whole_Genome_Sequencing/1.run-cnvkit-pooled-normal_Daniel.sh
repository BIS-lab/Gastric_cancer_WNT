...
access='../99.Reference/access.human_g1k_v37_decoy.bed' # WGS target bed comes from cnvkit "assess" command
csv_path='./sarek_Daniel/csv/recalibrated.csv'
outdir='result-pooled-normal-Daniel'
...
cnvkit.py batch $tumor_bam_paths --normal $normal_bam_paths --method wgs --annotate "$anno" -g "$access" -d "$batch_output_prefix" --rscript-path "$rscript_path" -p 100